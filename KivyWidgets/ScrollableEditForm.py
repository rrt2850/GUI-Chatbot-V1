...
class ScrollableForm(BoxLayout):
    def __init__(self, on_button_press, formNames=[], formText=[], **kwargs):
        ...
        for i in range(len(formNames)):  # change the range to add more TextInput fields
            ...
            formText[i] = formText[i] if formText[i] is not None else ""  # add this line
            text_input = MyTextInput(text=formText[i])  # create a TextInput instance

            # Store the text input instance in the dictionary
            self.text_inputs[formNames[i]] = text_input

            layout.add_widget(label)  # add Label to the layout
            layout.add_widget(text_input)  # add TextInput to the layout
